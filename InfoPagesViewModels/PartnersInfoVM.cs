    ...
    public class PartnersInfoVM : BindableBase
    {
        ...
        private void SaveChanges()
        {
            ...
            if (editName != string.Empty && editUNP.Replace(" ", string.Empty).Length != 9)
            {
                ...
                dataBase.Edit(partners[selectedPartner].Id, partner);
                partners = dataBase.GetList();
                RaisePropertyChanged(nameof(partners));
                EditCancel();
            }
        ...
        private void Delete()
        {
            dataBase.Delete(partners[selectedPartner]);
            partners = dataBase.GetList();
            RaisePropertyChanged(nameof(partners));
            EditCancel();
        }
        ...
        {
            if (selectedPartner == -1) return;
            selectedTabIndex = 2;
            var partner = partners[selectedPartner];
